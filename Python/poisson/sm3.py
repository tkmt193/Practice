...
for node_total in range(2,n_max):
...
n_x = np.arange(2,n_max,1) #思考回数

#mseをプロット
plt.plot(n_x,mse_total, label="${MSE}$", color='#ff0000')  #折線グラフを作成
plt.scatter(n_x,mse_total)  #点グラフを作成

#rmseをプロット
plt.plot(n_x,rmse_total, label="${RMSE}$", color='#0000ff')  #折線グラフを作成
plt.scatter(n_x,rmse_total)  #点グラフを作成
